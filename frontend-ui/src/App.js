  ...
  return (
    <div style={{ marginTop: 18 }}>
      <div style={{ display: "flex", justifyContent: "space-between", fontSize: 10, color: "#475569", fontFamily: "var(--mono)", marginBottom: 6 }}>
        <span>0%</span>
        <span style={{ color: hex, fontWeight: 500 }}>{pct.toFixed(1)}%</span>
        <span>100%</span>
      </div>
    ...
    <div style={{ marginBottom: 14 }}>
      ...
      <input
        ...
        style={{
          width: "100%",
          background: "#030810",
          border: `1px solid ${focus ? "#2563eb" : "#111c2e"}`,
          borderRadius: 9,
          padding: "11px 13px",
          color: "#e2e8f0",
          fontSize: 14,
          ...
          boxShadow: focus ? "0 0 0 3px #2563eb14" : "none",
        }}
      />
...
const TT = ({ active, payload }) => {
  ...
  return (
    <div style={{ background: "#080f1c", border: "1px solid #111c2e", borderRadius: 8, padding: "9px 13px", fontSize: 11, fontFamily: "var(--mono)", boxShadow: "0 8px 32px #00000080" }}>
      ...
      <div style={{ color: d.impact > 0 ? "#f87171" : "#4ade80" }}>
...
export default function App() {
  ...
  const analyze = async () => {
    ...
    } catch { alert("Backend not reachable — start uvicorn on port 8000."); }
    setLoading(false);
  };
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 18 }}>
                ...
                <div style={{ display: "flex", gap: 14, fontSize: 10, color: "#334155" }}>
                  <span style={{ display: "flex", alignItems: "center", gap: 5 }}>
                    <span style={{ width: 7, height: 7, borderRadius: 2, background: "#ef4444", display: "inline-block" }} /> RISK
                  </span>
                  <span style={{ display: "flex", alignItems: "center", gap: 5 }}>
                    <span style={{ width: 7, height: 7, borderRadius: 2, background: "#22c55e", display: "inline-block" }} /> SAFE
                  </span>
                </div>
              </div>
              <ResponsiveContainer width="100%" height={260}>
                <BarChart data={chart} layout="vertical" margin={{ left: 145, right: 20, top: 2, bottom: 2 }}>
                  ...
                  <Bar dataKey="impact" radius={[0, 4, 4, 0]} maxBarSize={20}>
                    {chart.map((e, i) => (
                      <Cell key={i} fill={e.impact > 0 ? "#ef4444" : "#22c55e"} fillOpacity={0.88} />
                    ))}
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
          ...
          {expls.length > 0 && (
            <div style={{ background: "#060d1a", border: "1px solid #0a1628", borderRadius: 15, padding: "26px 24px", marginTop: 18, animation: "fadeUp .4s .2s both" }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 7 }}>
                {expls.filter((e) => Math.abs(e.impact) > 0.01).slice(0, 8).map((e, i) => (
                  <ExCard key={i} {...e} />
                ))}
              </div>
            </div>
